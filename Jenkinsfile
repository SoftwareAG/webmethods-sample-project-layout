    ...
    node('mypod') {
        ...
        stage ('Deploy') {
            container ('helm') {
                ...
                def API_ID = sh ( script: "curl -u Administrator:manage -X POST 'http://api-gateway:5555/rest/apigateway/apis' -H 'accept: application/json' -H 'Content-Type: multipart/form-data' -F 'file=@bookstore.swagger' -F 'apiName=Bookstore' -F 'apiDescription=Bookstore API' -F 'apiVersion=V3' -F 'type=swagger' | jq -r '.apiResponse.api.id'",returnStdout:true).trim()
                sh "curl -u Administrator:manage -X PUT 'http://api-gateway:5555/rest/apigateway/apis/'${API_ID}'/activate' -H 'accept: application/json' -H 'Content-Type: multipart/form-data'"
            }
        }
    }
}
