    ...
    node('mypod') {
        ...
        stage ('Deploy') {
            container ('helm') {
                ...
                def tag = "2787512"
                sh "helm -n development list"
                sh "helm -n development upgrade --install --force --wait --timeout=600 --set image.repository=${repository},image.tag=${tag} softwareag-bookstore softwareag-bookstore"
