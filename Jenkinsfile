    ...
    node('mypod') {
        ...
        stage ('Deploy') {
            container ('helm') {
                ...
                sh "api-id=\$(curl -u Administrator:manage -X PUT 'https://api.devopsinitiative.com/rest/apigateway/apis' -H 'accept: application/json' -H 'Content-Type: multipart/form-data' -F 'file=@bookstore.swagger' -F 'apiName=Bookstore' -F 'apiDescription=Bookstore API' -F 'apiVersion=V3' -F 'type=swagger' | jq -r '.apiResponse.api.id'  ) echo \${api-id}"
            }
        }
    }
}
